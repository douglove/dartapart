...
public class PlayerView extends JPanel implements ScoreListener {
    ...
    public PlayerView(DAPlayer player) {
        try {
            m_Player = player;
    ...
    public void paint(Graphics g) {
        ...
        if (m_Player != null && m_Player.isEliminated()) {
        ...
        if (m_DisplayType != DISPLAY_NONE) {
          ...
          if (isScrolling()) {
            ...
            String sScore = (score == Integer.MAX_VALUE) ? "-" : Integer.toString(score);
            drawScore(g, leftInset, rightInset, rowHeight * 8, rowHeight * 9, sScore);
    ...
    private void drawScore(Graphics g, int startX, int endX, int startY, int endY, String score){
        drawString(g, startX, endX, startY, endY, score);
    }
