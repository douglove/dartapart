...
public class DartBoardViz
  ...
  private static final String [] LABELS = new String[] {"3", "17", "2", "15", "10", "6", "13", "4", "18", "1", "20", "5", "12", "9", "14", "11", "8", "16", "7", "19"};
  ...
  private Color m_clrBG    = new Color(100, 136, 110);
  ...
  public void addDart(double x, double y) {
    m_vecDarts.add(new Point2D(x, y));
  }
  ...
  public void paint(Graphics g) {
    ...
    int iLabelRingRadius = scale(DartBoardDimensions.PERCENT_LABELS);
    ...
    for (int i = 0; i < 20; i++) {
      int degs = (i * 18);
      int x = (int) (Math.sin(Math.toRadians(degs)) * iLabelRingRadius);
      int y = (int) (Math.cos(Math.toRadians(degs)) * iLabelRingRadius);

      Rectangle2D r = metrics.getStringBounds(LABELS[i], g2);

      int halfwidth  = (int) (r.getWidth() / 2);
      int halfheight = (int) (r.getHeight() / 3);

      g2.drawString(LABELS[i], x - halfwidth, y + halfheight);
    }
  ...
  private void addManualDart(double x, double y) {
    Point2D dart = new Point2D(x, y);
    ZoneConverter zc = new ZoneConverter();
    Zone z = zc.zone(dart.x * DartBoardDimensions.MM_BOARD, - dart.y * DartBoardDimensions.MM_BOARD);
    fireDartEvent(z);
  }
